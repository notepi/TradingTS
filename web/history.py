...
from web.translation import load_translated_text, translated_path
...
def _read_report_text(run_dir: Path, relative_path: str | Path, language: str | None = None) -> str | None:
    # Try translated version first if language is set
...
def _resolve_report_file(run_dir: Path, language: str | None = None) -> str | None:
    ...
    if language and language.strip().lower() != "english":
        translated = translated_path(run_dir, language, "complete_report.md")
        if translated.exists():
            return str(translated.resolve())
    # Fall back to original English
